...
Applied SHAP (GradientExplainer, after DeepExplainer failed an internal additivity check) to the Autoencoder, wrapped to output reconstruction error as a single explainable value.

1. **Top drivers identified**: Packet Length Variance, FIN Flag Count, and Bwd Packet Length Std dominate the model's anomaly scoring, confirmed consistently across three independent visualization methods (beeswarm, waterfall, bar chart).
2. **Single-flow waterfall example**: one flagged flow (score 8.675) was driven almost entirely by Packet Length Variance alone (+7.61 of the total score), with 64 other features contributing negligibly — demonstrating that individual anomalies are often explainable via a small number of dominant features.
3. **Forensic value confirmed**: an analyst can identify the specific behavioral driver behind a flag rather than treating detection as an opaque score.

### Adversarial Robustness Testing

Tested evasion resistance across four attack strategies of increasing sophistication.

1. **SHAP-targeted, random noise, and full-replacement attacks**: all achieved 0% evasion across 2,000 attacked flows.
2. **Gradient-based white-box attack**: achieved 35.3% evasion (704/2,000 flows), representing the theoretical ceiling of attacker capability with full model access.
3. **Root cause of the 0% results**: the detection threshold (80th percentile of benign reconstruction error) is aggressive by design — even the average benign flow (error 0.047) sits above it. This makes evasion difficult against realistic attackers, but is a direct trade-off with false-positive rate, not an unqualified strength.
4. **Conclusion**: FEAR resists realistic, low-to-medium sophistication evasion but retains a quantified, non-trivial vulnerability (35.3%) against a fully-informed adversary — a precise, defensible robustness boundary rather than a blanket claim.

### Streamlit Dashboard

Built a four-section analyst-facing dashboard: project overview and model comparison, filterable flagged-flows table, click-to-explain SHAP waterfall panel, and a robustness/limitations summary.

1. Reuses precomputed results (`flagged_results.csv`, `shap_values.csv`) rather than running inference live, keeping the dashboard fast.
2. Displays a curated 100-row sample (50 correct detections, 20 missed anomalies, 30 benign) — deliberately including misses for an honest picture, not just successes.

Week 4 — Packaging & Reporting

### Container-Friendly Refactor (Docker Prep)

Replaced hardcoded VM-specific paths (`/home/vboxuser/FEAR/...`) with a centralized `config.py` using `pathlib`, resolving all paths relative to the project root.

1. Dashboard updated to import paths from `config.py` instead of hardcoded strings — verified working after fixing syntax errors introduced during editing.
2. This is a prerequisite for Dockerization: hardcoded machine-specific paths would break inside a container regardless of volume mounts.
3. Next steps: `requirements.txt`, Dockerfile, docker-compose.yml with `data/` and `models/` mounted as volumes (not baked into the image, to keep it lightweight and avoid rebuilds on data/model changes).
